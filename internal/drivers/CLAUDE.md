...
| `GeyserDriver` | `geyser.go` | `NewGeyserDriver(accessKey, secretKey, bucket, tenantID, logger, ...GeyserOption)` | Spectra Logic Vail (LTO-9 tape) | Default LA endpoint; `WithGeyserEndpoint` for London; 64 MB spill threshold (RAM vs temp file); generous HTTP timeouts for tape ops; **HTTP/1.1 forced** (`WithHTTP1Only` — Vail negotiates h2 and Go then funnels all uploads through one TCP conn, capping aggregate at ~27 MB/s vs 227+ MB/s over h1; per-connection is ~25-30 MB/s, parallelize objects for throughput); **V18.2 recall**: implements `engine.Restorer` — `geyserWireErr` maps Vail's `InvalidObjectState` → `engine.ErrArchived` (Get/GetRange never surface a raw 500 for evicted objects) and `RestoreAlreadyInProgress` → sentinel; `RestoreObject(days)` + `RestoreStatus` (HEAD `x-amz-restore` passthrough). Tests: geyser_restore_test.go (fake Vail via httptest) |
...
| `R2Driver` | `r2.go` | `NewR2Driver(accountID, accessKey, secretKey, jurisdiction, bucket, logger)` | Cloudflare R2 | **Public buckets / CDN origin ONLY — not a tier** (SMART_TIER_DESIGN.md 2026-09-19). Fixed bucket (`R2_BUCKET`, default `vaultaire-public`) + `t-<tenant>/<container>/<artifact>` keys like iDrive/Geyser (R2 bucket names reject the `_` in container names, cap 1000); `R2Endpoint(acct, jurisdiction)` builds `<acct>[.<j>].r2.cloudflarestorage.com`; region `auto`, path-style, HTTP/1.1, **checksums `WhenRequired`** (R2 rejects the SDK's default streaming CRC trailers); parallel multipart via `s3ParallelUpload`; `GetRange`; signed HeadBucket health probe; `ObjectKey()`/`Bucket()` exposed for a future presigned/custom-domain serve path. Routed to only via the engine `PUBLIC` class. Live test `TestR2Driver_LiveRoundTrip` (R2_* env; verified 2026-09-24) |

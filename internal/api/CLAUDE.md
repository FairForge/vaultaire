...
- **server.go** — Server struct, router setup, middleware chain. **H-3:** `Shutdown` drains HTTP then synchronously flushes the three buffered trackers (bandwidth/CDN-analytics/access-log) — their goroutines run on `context.Background()` so the ticker's ctx.Done final-flush never fires; without this every deploy dropped up to 5s of buffered events (egress metering = billing data). Test: shutdown_flush_test.go (DB-backed; note `bandwidth_usage_daily.tenant_id` FK → seed a real tenants row, flushers swallow insert errors). **R1 (2026-09-26):** the signal path in `cmd/vaultaire/main.go` now runs `server.Shutdown` (drain + flush) BEFORE `engine.Shutdown` (which closes the shared `*sql.DB`), and treats `http.ErrServerClosed` as "stop sequence started" rather than fatal — `ListenAndServe` returns it the instant the listener closes, before in-flight requests drain. Tests: `cmd/vaultaire/main_test.go`. Client IP for the allowlist/access log comes from `internal/clientip` (`extractClientIP` delegates); never read `X-Forwarded-For`/`CF-Connecting-IP` directly
- **s3.go** — Request parsing, auth, routing to operation handlers
- **s3_errors.go** — Error codes, messages, and response writing
- **not_found.go** — `isObjectMissingErr`: classifies engine errors as object-miss vs backend-failure across ALL driver error shapes (typed `engine.NotFoundError` value or pointer, `os.ErrNotExist`, local-driver path errors, aws-sdk-go-v2 chains checked **by type** — `smithy.APIError` code `NoSuchKey`/`NotFound`, `*smithyhttp.ResponseError` HTTP 404 — plus the SDK's v2 text as a fallback). Used by GET/DELETE handlers so a miss on an S3-class backend maps to 404 `NoSuchKey`, never 500 (prod bug found 2026-07-31). **Precedence (R6-02):** anything wrapping `engine.ErrAllBackendsUnavailable` is never a miss — the recorded backend was unreachable; handlers answer 503 + Retry-After (GET, PUT and DELETE all check the sentinel first). Mirrors `engine.isBackendFailure` — keep in sync. `HandleDelete` reads `backend_name` with `is_chunked` and hints the engine before `engine.Delete` (R6-05: without the hint every post-restart DELETE of an off-primary object went to the primary, was answered "not found", dropped the head row and orphaned the bytes). Tests: not_found_test.go, s3_delete_routing_test.go
- **s3_engine_adapter.go** — GET/PUT/DELETE/LIST handlers bridging S3 to engine; `handleChunkedPut` streams uploads through `ChunkContext` for bounded-memory chunking + dedup (on failure returns 5xx, never falls through to normal path); since WP-ChunkPerf new-chunk stores fan out to a bounded worker pool (`s3_chunked_put_pool.go`, default 8 workers, env `CHUNK_PUT_CONCURRENCY`, 1 = sequential) — the dispatcher keeps all dedup decisions sequential, workers run compress→encrypt→`storeChunkLocked`, in-request same-hash races are prevented by an in-flight waiter map, manifest refs are re-sorted by chunk index before install, and peak memory is bounded by workers + chunker buffer (≤ ~19 chunks of ≤ 16 MB) regardless of object size, `handleChunkedGet` streams chunked objects on GET with bounded prefetch (since WP-ChunkPerf: up to `CHUNK_GET_PREFETCH` chunks, default 4, fetch+verify ahead of the write cursor on per-index buffered channels — a slot is held from fetch-start to writer-consume so buffered chunks never exceed the prefetch depth; the writer consumes strictly in chunk-index order, so ordering and error semantics are unchanged; 1 = sequential escape hatch) (`fetchAndVerifyChunk`: bounded ~16 MB buffer + SHA-256 integrity check per chunk; range via `chunk_offset`; corrupt first chunk → 500, never serves bad bytes). Chunks live in the shared `_global` container (const `chunkContainer`) so cross-bucket/cross-tenant dedup is retrievable. **WP-7 dedup scope**: the GCI is partitioned by `dedup_scope` — unencrypted chunks stay `_global` (cross-tenant dedup, stored `_chunks/{hash}`); encrypted chunks (per-chunk convergent, `ENCRYPTION_MASTER_KEY` set) scope to the tenant ID and store at `_chunks/{tenantID}/{hash}`, so identical plaintext from two tenants makes distinct scoped rows sealed with each tenant's key (no cross-tenant ciphertext leak). Scope is recorded on each `tenant_chunk_refs` row and read back on GET/DELETE/GC. The encrypted blob's `ciphertext_hash` is recorded once on the GCI row at first store; dedup hits copy it from the row (never recompute — the compression decision consults the request's Content-Type, so recomputing under a different Content-Type hashed a blob that was never stored and bricked the object's GETs). Whole-object SSE-S3 is skipped when this per-chunk path handles the object (avoids double-encryption + preserves chunk determinism).
...
- **s3_buckets.go** — CreateBucket (with region; A1: 200 carries `Location: /{bucket}` like AWS), DeleteBucket, ListBuckets, GetBucketLocation, HeadBucket
- **s3_acl.go** — `?acl` sub-resource (A1 conformance, 2026-09-18). We have no ACL model (every bucket/object is tenant-owned = AWS BucketOwnerEnforced): GET returns a canned `AccessControlPolicy` (owner + one FULL_CONTROL grant, `xsi:type="CanonicalUser"`); PUT accepts only owner-equivalent ACLs (absent/`private`/`bucket-owner-full-control` canned, or body grants that are all FULL_CONTROL) and refuses everything else 400 `AccessControlListNotSupported` — never silently pretends a public-read grant took effect. Before this file, `?acl` was unrouted: GET bucket?acl fell through to ListObjects (SDKs parsed the listing as an empty ACL) and **PUT object?acl fell through to PutObject and overwrote the object's bytes with the ACL XML**. Tests: s3_a1_conformance_test.go
- **s3_content_encoding.go** — PUT header round-trip family (A1, migration 061): `requestContentEncoding` (strips the `aws-chunked` transport token — aws-cli v2 sends `aws-chunked[,gzip]` on every streaming upload; the framing is decoded before storage so it must never be echoed back), `requestContentLanguage`, and `requestEchoHeaders`/`setEchoHeaders` for Cache-Control, Expires (stored raw as `http_expires` — NOT an object TTL), and `x-amz-website-redirect-location` (stored-and-echoed metadata only; no website endpoint exists). All persisted on `object_head_cache`, returned on GET/HEAD (set before the range branch, chunked + plain paths). A stored per-object Cache-Control wins over GET's `private, no-cache` default (guarded at both GET sites). PUT responses also carry `x-amz-object-size` (newer AWS surface, SDK `PutObjectOutput.Size`). Before A1 all of these headers were silently dropped
...
- **s3_list.go** — ListObjectsV2 handler. A1 (2026-09-18): entries carry the per-object `StorageClass` via `engine.BackendToStorageClass(backend_name)` — the same mapping HEAD uses — so listings and HEAD can never disagree (local→REDUCED_REDUNDANCY, geyser→GLACIER; backup tools plan restores off the listed class)
...
- **s3_restore.go** — V18.2 minimum recall slice: `handleRestoreObject` (`POST /{bucket}/{key}?restore`, operation `RestoreObject`) with AWS Glacier wire semantics — 202 Accepted on a new recall, 409 `RestoreAlreadyInProgress`, 403 `InvalidObjectState` for hot-backend objects, 404 for missing keys. Resolves the object's backend from `object_head_cache` and type-asserts the driver to `engine.Restorer` (only Geyser implements it). Days parsed from the XML body (default 1, `GlacierJobParameters` accepted-and-ignored). GET on an archived object → 403 `InvalidObjectState` (engine.ErrArchived branch in `HandleGet`); HEAD on GLACIER-class objects does a live `RestoreStatus` passthrough and sets `x-amz-restore` — the ONE deliberate exception to "HEAD never touches the backend" (restore state only exists there; failure just omits the header). rclone/aws-cli restore workflows work unmodified. Tests: s3_restore_test.go (stub archive driver), drivers/geyser_restore_test.go (fake Vail wire).
...
- **security_txt.go** — `GET|HEAD /.well-known/security.txt` (RFC 9116, launch 5.5.6), registered before the S3 catch-all (it used to fall through and answer 403 AccessDenied). Rendered once at boot: `Contact: mailto:security@stored.ge`, `Expires` = boot day + 364d (must stay < 1 year), `Canonical`, `Policy` = `SECURITY_POLICY_URL` (default the AUP page). Test: security_txt_test.go (production router)
- **cert_expiry.go** — origin TLS certificate watchdog: `TLS_CERT_PROBE_TARGETS` (`sni@host:port`, comma list) → hourly verified TLS handshake (on expired/mismatched/unknown-CA the leaf is read from the x509 error, so it still reports) → `vaultaire_tls_cert_expiry_timestamp_seconds{sni}` + `vaultaire_tls_cert_probe_ok{sni}` on `/metrics`; a failed probe keeps the last good NotAfter so the expiry alert cannot un-fire on a flaky dial. Loop starts in `startHealthChecks`. Rules: `deploy/monitoring/vaultaire-tls.yml` (<14d warning, <7d critical, probe failing 2h). Why: the origin LE cert expires 2026-10-29 (T-2 to launch) and `certbot --dry-run` cannot prove the real renewal (staging CA → "Certificate not found"). Test: cert_expiry_test.go
...
- **openapi_drift_test.go** — B4 Tier-0 docs drift guard: walks the full production router (NewServer with nil DB) via `chi.Walk` and fails the build when an `/api/*` or `/auth/*` route is neither in the OpenAPI spec (`internal/docs.GenerateOpenAPISpec`) nor in the test's `knownUndocumented` allowlist (100 routes as of 2026-07-20 — the enumerated "7 of ~40" debt, B3 burns it down), when an allowlist entry goes stale (documented or deleted), or when the spec documents a path no route serves. New endpoints ship documented or are consciously allowlisted in review — silent drift breaks CI
...
## Object Lock error code (A1, 2026-09-18)

Lock-protected DELETE/overwrite answers **403 `AccessDenied`** (AWS's wire code) with "Object is protected by Object Lock." appended via `WithSuggestion` — the former invented `ObjectLocked` code broke SDK error handling and versitygw conformance. Internal sentinel `errObjectLocked` (s3_lock.go) is unchanged; only the wire code moved. Sites: s3_engine_adapter.go (PUT-overwrite + delete), s3_batch.go (batch delete per-key error).
...
**Bucket tier preference** (Phase 7.5, `resilient` added by WP-ResilientTier): `PUT /api/v1/manage/buckets/{name}/tier` (`handleMgmtSetBucketTier`) — sets the bucket's `tier_preference` column. Request body: `{"tier": "auto|performance|standard|archive|resilient"}`. Returns 400 for invalid tier, 404 for missing bucket, 200 with `tier_preference` in the response envelope. `resilient` maps to the internal `RESILIENT` storage class → Lyve backend (the $7.99 tier). Resilient-tier AND archive-class (`GLACIER`/`DEEP_ARCHIVE`, via bucket tier `archive` or explicit `x-amz-storage-class` header) objects **skip the chunked PUT path** (chunk blobs always land on the engine primary via the shared `_global` container, which would silently defeat the tier's placement promise — for archive that meant >64 MB objects got hot iDrive COGS under a $2/TB tape price) — objects store whole on Lyve/Geyser (deliberate trade: no dedup for these tiers), and multipart complete resolves the bucket tier class too (`s3_multipart.go` passes `bucketTierStorageClass` to the assembled-object Put, so aws-cli default multipart uploads respect tier placement; residual gap: an explicit `x-amz-storage-class` header at multipart *initiate* is not persisted — `multipart_uploads` has no storage_class column — so header-classed multiparts into an `auto` bucket land on the primary). Tests: s3_resilient_tier_test.go, s3_archive_tier_test.go.
...
## Smart-Tier Demotion Job (Phase 5.15.8, 2026-09-23)

- `smart_demotion.go` — `SmartDemotionRunner` (pattern = `dedup_gc.go`): daily `RunOnce(ctx, dryRun)`; admin `POST /api/v1/admin/smart-demotion[?dry_run=true]`. Per tenant on the managed tiers (default `standard`), hot budget = `HotFraction` (0.15) × `tenant_quotas.storage_limit_bytes`. Candidates: whole objects on `HotBackend` (idrive) older than `MinAge` (3d) in buckets that are neither versioned nor `tier_preference='performance'`; **idle** (`last_accessed` < now−14d, oldest first) always, then **LRU** extras only while still over budget. Per-run byte cap `MaxBytesPerRun` (500 GiB; every demoted byte crosses SLC).
- **Routing truth is `object_head_cache.backend_name`** (GET/HEAD/restore route on it). A demotion = copy hot→cold (driver-level, tenant in ctx, container `tenantID_bucket`) then an **etag-guarded conditional flip** of that column; 0 rows = the object changed during the copy → cold copy removed, skipped. Then `HintBackend` for this process.
- **Hot copy is reclaimed later, not at demotion**: ledger `smart_demotions` (062) records each move; after `HotGrace` (24h) the next run takes the head-cache row `FOR UPDATE` and deletes the hot blob only if the row still routes cold with the same etag (`deleted`); rewritten/moved since → `kept_changed` (hot bytes are live data); head row gone → `object_gone` (orphan removed); delete error → `delete_failed`, retried next run. Residual race: a PUT whose blob write lands during the ms-long guarded delete — same class as the pre-existing overwrite race, accepted + documented.
- **Flag**: `smart_demotion` (default OFF, per-tenant row or `'*'`), checked per tenant. Ships flag-dark; verify with `?dry_run=true` before enabling a tenant. Env knobs: `SMART_DEMOTION_HOT_FRACTION`, `_IDLE_DAYS`, `_MIN_AGE_DAYS`, `_MAX_GB_PER_RUN`, `_TIERS`.
- **Read-time promotion** (`smart_promotion.go`, `SmartPromoter`, wired into `HandleGet` via `adapter.smartPromoter`): when the head cache routes a whole object to the cold backend AND a `smart_demotions` row matches its etag — (a) hot copy still present (inside grace) → `OnRead` flips routing back to hot in one conditional UPDATE (+ `last_accessed=NOW()`), drops the ledger row and the cold copy: zero data movement; (b) hot copy reclaimed → this read is served from cold (Geyser staging) and an async copy-back cold→hot runs (bounded concurrency, in-flight dedupe, `promote_requested_at` in the ledger; the daily job's `PromotePending` retries leftovers); (c) the copy-back or the read hits `ErrArchived` (evicted to tape) → `RestoreObject(7d)` is submitted on the reader's behalf once (`restore_requested_at`) and the GET answers **503 + Retry-After: 120** instead of Glacier's 403 (Vault objects without a ledger row keep the 403). A promotion refreshes `last_accessed` so the next run cannot demote it straight back. Copy-back that finds the object changed never deletes a hot blob that the newer PUT may own.

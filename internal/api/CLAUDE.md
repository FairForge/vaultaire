...
- **Not in this WP (follow-up PR B)**: read-time promotion (within grace = routing flip back; after = copy cold→hot) and auto-restore-on-read for evicted objects. Until B ships a demoted+evicted object reads as GLACIER (403 InvalidObjectState + RestoreObject), so keep the flag OFF in prod. Chunked objects and versioned buckets are out of scope for v1.
- Tests: `smart_demotion_test.go` (DB-backed via `DATABASE_URL`; LocalDrivers stand in for idrive/geyser).

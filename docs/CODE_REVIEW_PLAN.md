...
| R6 | Engine core (routing, failover, tiering, replication) | R0 | **done 2026-09-26** — 27 findings (6 P1 fixed here: SDK-v2 404 casing + client-cancel breaker pollution, outage reported as 404, access-tracker relocating `auto`-bucket overwrites to Lyve, write failover onto r2/geyser/permafrost/EU regions, Delete ignoring the recorded location after restart; 1 P1 handed to R13 — access-log/inventory writers use the wrong container namespace; 9 P2, 11 P3 → 9 WPs); placement narrative for ARCHITECTURE.md + driver contract table for R7; cache decision = delete (WP-R0-3) | [R6-engine.md](reviews/R6-engine.md) | #478 |
| R7 | Storage drivers | R6 | not started | | |
| R8 | Crypto, chunking, GCI dedup, GC | R2 | not started | | |
| R9 | Database layer & migrations | R0 | **done 2026-09-26** — 25 findings (0 P0; 5 P1: three phantom-column/interval bugs that 500'd the management object list, silently emptied the GDPR export and broke `/api/v1/quota/history` since May — all fixed + `TestSQLLiteralsMatchSchema` now prepares every SQL literal against the migrated DB; unbounded `s3_access_log`/`events`/`quota_usage_events` growth (WP-R9-2); WP-R0-7 done — `InitializeSchema` deleted, quota tests on the migrated schema, 26 test files → `testutil.DSN()`; 10 P2 incl. `NewPostgres` never pings, a dedup-migrate test cleanup that wiped every chunk row (the CI flake root cause — fixed), runner has no lock_timeout, plain-text backups world-readable on the box; 10 P3); migrations proven idempotent (double apply, 0 errors) and prod schema == migrations; 39 orphan tables → decision D-12; schema-by-domain draft for R14; 10 WPs | [R9-database.md](reviews/R9-database.md) | PR_PLACEHOLDER |

...
Migration numbers in this plan are **planning references**, not assignments. Existing migrations go up to `062_smart_demotions.sql` (as of 2026-09-23; was `052_chunk_encryption.sql` on 2026-07-07). <!-- reconstructed: verified against internal/database/migrations/ --> The next migration built gets `063`, regardless of which phase it comes from. Assign sequentially at build time — multiple phases reference the same numbers but only one migration can have each number. When starting a phase, check `ls internal/database/migrations/` for the latest number and use the next one.
...
**STATUS 2026-09-23: PR A SHIPPED (demotion + ledger) — `internal/api/smart_demotion.go`, migration 062 `smart_demotions`, flag `smart_demotion` (default OFF, ships flag-dark), admin `POST /api/v1/admin/smart-demotion?dry_run=true`. Routing flips in `object_head_cache.backend_name` under an etag guard; hot copy reclaimed after a 24h grace on the next run (row-locked, etag-checked). v1 scope = whole objects, non-versioned buckets, `tier_preference!=performance`. **PR B (next): read-time promotion** (within grace = flip back, after = copy cold→hot) + auto-restore-on-read for evicted objects — keep the flag OFF in prod until B lands, otherwise a demoted+evicted object reads as GLACIER.**
...
4. Migration numbers: `ls internal/database/migrations/` and take the next sequential (`062_smart_demotions.sql` is the latest as of 2026-09-23).

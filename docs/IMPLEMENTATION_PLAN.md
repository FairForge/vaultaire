...
5. SLA copy (honest, measured): "restores begin within minutes" (recall <3 min idle — re-verify under contention before printing an SLA number); drain from staging parallelized across N workers, sized against the shared gateway. **Drain measured (2026-07-29): ~4-5 MB/s per stream, aggregates with parallel objects (3×256MB → 11.6 MB/s) toward the same ~27 MB/s-class gateway ceiling as ingest → rehydration throughput ≈ 2 TB/day fleet-wide.** So: progressive restore (serve each object the moment it lands hot, never wait-for-all), drain scheduler prioritizes user-requested objects first, and honest copy is "restores begin in minutes; bulk restores drain up to ~2 TB/day (shared)". Bulk recall re-verified at 24 objects (<3 min) and submission at 17.6 req/s.

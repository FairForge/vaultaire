...
| V18.2 | Restore orchestration + hot rehydration (InvalidObjectState → RestoreObject → rehydrate to hot tier; Glacier-compatible wire semantics) | **MINIMUM RECALL SLICE = PRE-LAUNCH (2026-08-04 reprice decision, condition 4)**: ErrArchived + 403 InvalidObjectState + RestoreObject passthrough + HEAD x-amz-restore + dashboard Restore button. Full orchestration (rehydration worker, bulk thaw, webhooks) stays **first post-launch WP** |
